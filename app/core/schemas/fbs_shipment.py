...
class FbsShipmentItemResponse(BaseModel):
    ...
    movement_id: Optional[int] = Field(None, description="ID созданного движения товара (заполняется при success)")
...
class FbsShipmentListItem(BaseModel):
    ...
    status: str = Field(description="Статус обработки: processing, completed, partially_completed, failed, validation_failed")
    error_message: Optional[str] = Field(None, description="Текст ошибки (для validation_failed — текст исключения Pydantic)")
    completed_at: Optional[datetime] = Field(None, description="Дата завершения обработки всех позиций")
...
class FbsShipmentDetailResponse(BaseModel):
    ...
    error_message: Optional[str] = Field(None, description="Текст ошибки (для validation_failed — текст исключения Pydantic)")
    completed_at: Optional[datetime] = Field(None, description="Дата завершения обработки всех позиций")
    items: List[FbsShipmentItemResponse] = Field(default=[], description="Позиции отгрузки (пустой список для validation_failed)")
...
class RetryResponse(BaseModel):
    ...
    still_failed: int = Field(description="Количество записей, оставшихся со статусом validation_failed")
