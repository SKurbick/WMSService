-- STATUS: UTILITY. Read-only diagnostic queries; no DDL or DML.
...
-- FBS ship movements без связи с shipment item (read-only audit)
SELECT
    m.movement_id,
    m.product_id,
    m.quantity,
    ...
    m.reason,
    m.created_at,
    COUNT(fi.item_id) AS linked_fbs_items_count
FROM wms.movements m
LEFT JOIN wms.fbs_shipment_items fi ON fi.movement_id = m.movement_id
WHERE m.movement_type = 'ship'
  AND m.user_name IN ('FBS-service', 'FBS 2.0')
GROUP BY m.movement_id, m.product_id, m.quantity, m.user_name, m.reason, m.created_at
HAVING COUNT(fi.item_id) = 0
ORDER BY m.created_at DESC;

-- Success FBS items без movement link (ожидается 0 строк)
SELECT item_id, shipment_id, product_id, quantity, status, movement_id,
       error_message, created_at, updated_at
FROM wms.fbs_shipment_items
WHERE status = 'success' AND movement_id IS NULL;
