...
> **Статус: HISTORICAL.** Это gap-аудит на дату составления, а не актуальная
> карта API. Текущий контракт проверяйте по `api_map.md`, routers и OpenAPI.

Источник: `docs/current/api_map.md`, `app/api/v1/endpoints/`, SQL/service context in `app/infrastructure/database/queries/`, `known_issues.md`, `write_operations_policy.md`.
...
| POST | `/api/fbs-shipments/items/{item_id}/retry` | Ручной retry одной FBS item | write | `wms.fbs_shipment_items`, `wms.movements`, `wms.inventory`, `public.assembly_task` | high |
