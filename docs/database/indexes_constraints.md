...
> **Статус: CURRENT.** Snapshot и миграции сверены с read-only runtime-аудитом 2026-08-08.
...
Runtime содержит оба movement checks как `NOT VALID`; найдена одна legacy quantity violation и нет side violations.

Отсутствуют: `task_items.quantity_planned > 0`; `notifications.severity`; `locations.level` range; FK/check для корректности `container_code`.
